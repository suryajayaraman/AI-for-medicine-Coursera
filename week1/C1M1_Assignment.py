...
def get_test_and_valid_generator(valid_df, test_df, train_df, image_dir, x_col, y_cols, sample_size=100, batch_size=8, seed=1, target_w = 320, target_h = 320):
    ...
    print('Batch[0] type is ', type(batch[0]))
    print('Batch[0] shape is ', (batch[0].shape))
...
train_generator.labels.shape, len(train_df)


# In[15]:


valid_generator.labels.shape, len(valid_df)


# In[16]:


test_generator.labels.shape, len(test_df)
...
def get_weighted_loss(pos_weights, neg_weights, epsilon=1e-7):
    ...
    def weighted_loss(y_true, y_pred):
        ...
        #y_pred = epsilon
        positive_loss = -1. * K.sum(K.mean((pos_weights * y_true * K.log(y_pred + epsilon)), axis =0))
        negative_loss = -1. * K.sum(K.mean((neg_weights * (1 - y_true) * K.log(1 - y_pred + epsilon)), axis =0))
